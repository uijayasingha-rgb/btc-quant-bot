...
def fetch_crypto_quant_data(symbol="BTCUSDT", interval="5", limit=300):
    """
    Bybit Public API endpoint - High stability for Cloud Servers (Railway/AWS)
    """
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if raw_list and len(raw_list) > 0:
                # Bybit returns data newest to oldest, so reverse it
                raw_list.reverse()
                ...
                # Estimate taker buy volume as ~52% proxy for footprint delta if exchange taker data is hidden
...
def train_5m_quant_model():
    print("⏳ Fetching Data and Training AI Model...", flush=True)
    df = pd.DataFrame()
    attempts = 0
    while True:
        attempts += 1
        ...
        if not raw_df.empty and 'close' in raw_df.columns:
            ...
            if not df.empty and 'close' in df.columns and len(df) >= 30:
                ...
                break
        print(f"⚠️ Data Fetching Retry ({attempts})...", flush=True)
        time.sleep(5)
...
# Start background Quant Execution Thread
