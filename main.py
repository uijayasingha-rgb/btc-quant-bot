...
def fetch_crypto_quant_data(symbol="BTCUSDT", interval="5", limit=200):
    # Public Bybit API - Never blocks cloud hosting IPs
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if list_data and len(list_data) > 0:
                ...
                # Estimate taker buy volume as ~52% for delta estimation if not explicit
    ...
    while True:
        ...
        if not raw_df.empty and 'close' in raw_df.columns:
            ...
            if not df.empty and 'close' in df.columns and len(df) >= 30:
                print("✅ Market Data Received Successfully via Bybit API!", flush=True)
                break
        print(f"⚠️ Fetching Retry... (Attempt {attempts})", flush=True)
        time.sleep(4)
