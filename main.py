...
def fetch_binance_quant_data(symbol="BTCUSDT", interval="5m", limit=300):
    # Multi-endpoint fallback system to bypass Cloud/Railway IP blocks
    endpoints = [
...
def train_5m_quant_model():
    ...
    df = pd.DataFrame()
    while True:
        raw_df = fetch_binance_quant_data(interval="5m", limit=300)
        if not raw_df.empty and 'close' in raw_df.columns:
            ...
            if not df.empty and 'close' in df.columns and len(df) >= 30:
                print("✅ Binance Data Received Successfully!", flush=True)
                break
        print("⚠️ Retrying Data Fetch...", flush=True)
        time.sleep(3)
