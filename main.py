...
def fetch_crypto_quant_data(symbol="BTC", interval="5", limit=300):
    """
    CryptoCompare API with Full Error Logging
    """
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if raw_list and len(raw_list) > 0:
                ...
                print(f"⚠️ Empty Data List from API: {data}", flush=True)
        else:
            print(f"❌ API HTTP Error Code: {res.status_code}", flush=True)
    ...
    while True:
        ...
        if not raw_df.empty and 'close' in raw_df.columns:
            ...
            if not df.empty and 'close' in df.columns and len(df) >= 30:
                ...
                break
        print(f"⚠️ Data Fetching Issue. Retrying in 5s...", flush=True)
        time.sleep(5)
